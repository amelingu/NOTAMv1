...
function isMappable(n) {
  if (isProcedureChange(n)) return false;
  // Runway/taxiway/AD closures are never map obstacles regardless of text content
  const q = (n.itemq || '').toUpperCase();
  if (/\/QM[RTA][LC]/.test(q)) return false;  // QMRLC, QMTLC, QMALC etc.
  if (isFullClosure(n)) return false;
...
function isFullClosure(n) {
  ...
  return /RWY[\s\w\/]+CLS[DE]D?/.test(e) || /RUNWAY\s+CLS[DE]D?/.test(e) ||
    ...
    // French
    /PISTE[\s\w\/]+FERM[EÉ]E?/.test(e) || /\bAD\s+FERM[EÉ]/.test(e) ||
    /AÉRODROME\s+FERM[EÉ]/.test(e) || /AERODROME\s+FERM[EÉ]/.test(e);
}

// ── NOTAM colour ───────────────────────────────────────────────────────────
const CLOSURE_RE = /\b(CLSD|CLOSED|U\/S|UNSERVICEABLE|NOT AVBL|NOT AVAILABLE|OUT OF SERVICE|PROHIBITED|SUSPENDED|FERM[EÉ]E?|INDISPONIBLE|HORS SERVICE)\b/;
...
async function go() {
  ...
  try {
    ...
    [...new Set([...routeSeq, ...exs])].forEach(icao => {
      ...
      else missingCoords.push(icao);
    });
    // Warn prominently about any ICAOs not found in the airport database
    // (closed airports, typos, non-ICAO codes) — corridor and nearby scan
    // will be incomplete without their coordinates.
    if (missingCoords.length > 0) {
      const warn = document.createElement('div');
      warn.id = 'missing-coords-warn';
      warn.style.cssText = 'background:#FFF8E1;border-left:4px solid #F9A825;padding:8px 12px;'
        + 'margin-bottom:8px;font-size:11px;color:#555;border-radius:4px;'
        + '-webkit-column-span:all;column-span:all';
      warn.innerHTML = '⚠ <strong>Airport(s) not found in database:</strong> '
        + missingCoords.map(i => `<strong>${i}</strong>`).join(', ')
        + ' — no corridor scan or nearby airports for these. '
        + 'The airport may be closed, renamed, or the ICAO code may be incorrect. '
        + '<a href="https://ourairports.com/airports/" target="_blank" '
        + 'style="color:#1565C0">Check OurAirports</a>.';
      const results = document.getElementById('results');
      // Insert at top of results, before existing content
      const existing = document.getElementById('missing-coords-warn');
      if (existing) existing.remove();
      if (results) results.prepend(warn);
    } else {
      const existing = document.getElementById('missing-coords-warn');
      if (existing) existing.remove();
    }
